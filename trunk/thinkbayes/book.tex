...
Euro problem.  At last the time has come.

Remember that in the Belgian Euro problem we try to estimate the
probability, denoted {\tt x}, that a coin spun on edge lands heads up.
For data we have the results of 250 spins: 140 heads and 110 tails.

The goal is to compute the distribution of {\tt x}; that is, for
any given value, denoted {\tt y}, we would like to know the probability
that the true value of {\tt x} is {\tt y}.
...
chapter we will use a continuous distribution, specifically the beta
distribution (see
...
likelihood function, which is what we did in Section~\ref{optimization},
the beta distribution is a ``conjugate prior.''

That means that if the prior distribution for {\tt x} is a beta
distribution, the posterior is also a beta distribution.
But wait, it gets even better.

The shape of the distribution depends on two parameters,
conventionally written $\alpha$ and $\beta$, or
...
So let's see how we can take advantage of all this.  Here's
...
{\tt EvalBinomialPmf}, {\tt p} is the parameter, and {\tt yes} and
