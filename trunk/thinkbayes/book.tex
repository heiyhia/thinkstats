...
\chapter{Books and buses}

\section{The hockey problem}

Suppose that my beloved Boston Bruins are playing ice hockey against
the despised Buffalo Sabres.  After the first period (of three), the
score is Boston 1, Buffalo 0.  What is the probability that the
Bruins will win?
...
game.  Second, we can assume that when team A plays team B, each time
has some long-term average rate of goal scoring, denoted $\lambda$;
for convenience I will measure $\lambda$ in goals per game.
...
\item Use the score from the first period to estimate $\lambda$
for each team.
...
normal with mean 2.7 and standard deviation 0.3.
...
\section{The Green Tea Press problem}

When I started publishing my own books, I had to decide how many
copies of the book to print.  Printing a small number of books
is relatively expensive; when you print in bigger batches, the
per-unit cost can be much less.  But if you order too many books,
inventory costs can add up, and you might end up with unsold stock.
So it is important to be able to forecast sales.
