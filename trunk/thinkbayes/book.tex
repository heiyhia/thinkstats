...
is demonstrably correct and check whether it is fast enough for
my purposes.  If so, there is no need to optimize.

But in this case there are two ways we can speed it up substantially.
In the original code, we call \verb"Update" once for each coin toss.
...
even more by rewriting \verb"Likelihood".  In the original version,
...
Alternatively, we could pass the entire dataset as a tuple of integers
encoding the number of heads and tails.  In that case 
\verb"Likelihood" looks like this:
...
this version takes the same time for any number of coin tosses (up
to a point).
