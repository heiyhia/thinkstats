...
And that brings us back to the original question, ``How strong is the
...
Because there are only two hypotheses, I was able to
simplify the code.
...
The likelihood of $A$ is 0.38 and the likelihood of $B$ is 0.08.
The likelihood ratio is 4.5, which means that these test scores are
evidence that Alice is actually better than Bob at answering
SAT questions.  If we believed, before seeing the test scores,
that $A$ and $B$ were equally likely, then after seeing the scores
we should believe that the probability of $A$ is 82\%, which
means there is still an 18\% chance that Bob is actually better
prepared.
...
But remember that the analysis we have done so far is based on
...
To determine the effect of questions with different difficulty,
...
\item Finally, assume that the chance that a test-taker answers
a question correctly is related to $e$ and $d$ according to this
function:
...
in two steps.  First, for person with given {\tt efficacy},
...
{\tt dist} is the sum of these Pmfs.  Remember from Section~\ref{}
...
of raw scores.  For a group of people with a different
efficacies, the resulting distribution of raw scores is a mixture.
Here's the code that computes the mixture:
...
{\tt efficacies} is a Pmf that represents the distribution
of efficacy.  I assume that it is Gaussian with mean 0 and
standard deviation 1.5.  This choice is mostly arbitrary.
The probablility of getting a question correct depends on
the difference between efficacy and difficulty, so we can
choose the units of efficacy and then calibrate the units
...
So now if we are given the distribution of difficulty, we
can compute the distribution of raw scores.  But in reality
it is the other way around: we are given the distribution of
...
So we infer that if the distribution of difficulty is uniform,
...
So someone with efficacy 3, which is two standard deviations above
...
When we compute {\tt ProbCorrect}, we only care about the
difference


The units of efficacy are arbitrary;



because
efficacy and difficulty are on the same scale, we only care
about the difference between them.  So I can choose the
efficacy scale and find the 





Before we can proceed, we have to calibrate the model by choosing
distributions for {\tt efficacy} and {\tt difficulty} that yield
a distribution of scores close to what we actually observed.

  In order
to model the scenario, we
