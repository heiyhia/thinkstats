...
Here {\tt data} needs to be a sequence of counts, the same length
as {\tt params} and in the same order.  So in this example, it
should be the number of lions and tigers and bears, in that order.
...
There are two good ways to generate a random sample from
a Dirichlet distribution.  One option is to use the marginal
beta distributions, but in that case you have to select one
at a time and scale the rest so they add up to one.

A less obvious, but faster, way is to select values from {\tt n} gamma
distributions with parameters {\tt (params[i], 1)} (see
