...
Figure~\ref{} shows the results.  As expected, the maximum
likelihood estimates are 50\%, 33\% and 17\%.
...
In this example, we have seen 3 species, so there have to be at
least that many.  I chose an upper bound that seemed reasonable, but
we can come back an confirm that the probability of exceeding this
bound is low.
...
{\tt Likelihood} get a hypothesis and a dataset as arguments:
...
Why do we have to call it 1000 times?  Because {\tt Dirichlet.Likelihood}
doesn't actually compute the likelihood of the data under the
Dirichlet distribution.  Rather, it draws one sample from the
hypothetical distribution and compute the likelihood of the
data under the sampled set of prevalences.  
...
we see more species than we though existed, the likelihood is 0.
...
where $p_i$ is the prevalence of the $i$th species, and $x_i$ is the
...
To get the distribution of {\tt n}, {\tt DistOfN} iterates through the
...
Figure~\ref{} shows the result.  The most likely estimate is that
there are 3 three species, but after only 6 observations there is
non-negligible probability that there are several unseen species.

The probability that there are 9 species in total is quite small,
so the upper bound we chose seems reasonable.  But remember that
we started with a uniform prior for {\tt n}.  If we have background
information about the number of species in the environment, we might
choose a different prior.
...
for 1000 samples from the Dirichlet distribution.  {\tt self.probs}
is multiplied by the total likelihood, then normalized.

Updating the parameters is identical to what we saw in {\tt
  Dirichlet.Update}.
...
But wait, there's more!  Now that we've made the computation nicely
rectangular, we can re-write the loop as an array operation.
...
{\tt gammas}, {\tt row} and {\tt col} are unchanged, except that
we've selected just the elements of {\tt col} we need.

{\tt numpy.axis} makes a ``slice'' of {\tt col} that is two
dimensional, with the length of the new dimension being 1.
As a result, when we divide {\tt row} by {\tt col}, {\tt row}
gets ``broadcast,'' which means making a copy of {\tt row}
for each element in {\tt col}, and then dividing through.

The result is an array with one row of {\tt p} for each value
of {\tt n} in {\tt col}.  This operation is similar to a linear
algebra ``outer product,'' except in this case it's an outer
quotient.

{\tt terms} is an array of multinomial terms; when we add them
up, {\tt axis=1} says we want to add up the rows, so the result
is a 1-D array of the total log-likelihood for each value of n.

Finally, we shift the log-likelihoods into a comfortable range
before transform to linear likelihoods.

This version performs the same computation as the previous one,
but I expect it to run faster because it does less of the work
in Python, and more inside {\tt numpy}, which is written in C.

The price of all this optimization is that this version is even less
readable; nevertheless I confimed that this version yields the
