...
Thanks to Dirk Hoag at \url{http://forechecker.blogspot.com/},
I was able to get goals scored during regulation play (not overtime)
...
extreme values will be contradicted by evidence.  But if the
...
Basic estimate in multiple dimensions is easy; we just represent
each hypothesis with a tuple of parameters.  For the normal
distribution, the obvious tuple is {\tt mu, sigma}.

For this problem, I defined a Suite called Height that represents
...
The likelihood function is also easy.  Given hypothetical values
of {\tt mu} and {\tt sigma}, we have to compute the likelihood
of a particular value, {\tt x}.  That's what the Gaussian PDF
...
If you have studies statistics from a mathematical perspective,
...
But for our purposes here, we don't need a probability; we just
need something that is proportional to the probability we want.
A probability density does that job nicely.

That's pretty much it.  The hardest part of this problem turns
out to be choosing appropriate ranges for {\tt mus} and
{\tt sigmas}.  If the range is too small, we neglect some
possibilities with non-negligible probability, and get the
answer wrong.  If the range is too big, we get the right answer,
but we waste computational power.

So this is an opportunity to use classical estimation techniques
to make Bayesian techniques more efficient.  Specifically, we 
can use the sample to estimate {\tt mu} and {\tt sigma}, and use
the standard errors of those estimates to choose the range.

If the true, but unknown, parameters of the distribution are $\mu$ and
...
We chose a value for \verb"num_stderrs" big enough to
...
$(179, 8.7)$, and for women $(163, 6.2)$.


\section{The posterior distribution of CV}

These values are identical to the classical estimators, so if
that's all we care about, we haven't accomplished much.  The
advantage of the Bayesian approach is that with the posterior
distribution (not just a single point estimate), we can compute
the distribution of CV for men and women, and the 
probability that one exceeds the other.
...
Doesn't get much simpler than that.  Given the distribution of
...
that the probability is 96\%, but there are two problem with
this conclusion:

\begin{enumerate}

\item When we collapse the distributions to a single probability,
we lose information about uncertainty.  For example, if we report
``The probability that men are more variable is 96\%,'' that report
would not capture the likelihood of the Variability Hypothesis
before we saw the data or the weight of the evidence due to sample
size.  We can address these limitations using Bayesian hypothesis
testing, which is the topic of the next chapter.

\item The first 100 values are not a representative sample
from the dataset.  Of course, we should use the entire dataset.
...
\section{Big data, small probability}
...
If you take a large number of small probabilities and multiply
them together, the result is very small.  In this case it is
so small it can't be represented by a floating-point number, so
it gets rounded down to zero, which is called ``underflow.''

If all of the probabilities in the distribution are 0, it's not a
...
A better alternative is to compute probabilities under a log
transform.  That way, instead of multiplying probabilities, we can add
up log probabilities.  {\tt Pmf} provides methods {\tt Log}, {\tt
...
One advantage of working under the log transform is that for many
continuous distributions, it is easier (and a little faster) to
...
it is not a hashable type.  That's why {\tt LogUpdateSet} converts
the dataset to a tuple.

Together, these optimizations speed up the computation by about a
factor of 100, making it possible to process the entire dataset (154
