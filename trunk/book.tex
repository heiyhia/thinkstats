...
\beforefig
\centerline{\includegraphics[height=2.5in]{code/nsfg_birthwgt_pmf.eps}}
\afterfig
...
that is, dividing the data into non-overlapping ranges, called
``bins,'' and counting the number of points in each bin.  Binning can
be useful, but it is tricky to get the size of the bin right.  If
they are big enough to smooth out the noise, they might also smooth out
useful information.
...
The following figure shows the CDFs of birth weight
for first babies and others in the NSFG dataset.

\beforefig
\centerline{\includegraphics[height=2.5in]{code/nsfg_birthwgt_cdf.eps}}
\afterfig

This figure makes the shape of the distributions, and the differences
between them, much clearer.  We can see that...

reading percentiles from CDFs

\begin{ex}
Compute some percentiles and percentile ranks.
