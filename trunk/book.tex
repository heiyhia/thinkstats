...
weights in the NSFG.  Figure~\ref{nsfg_birthwgt_normal} shows the
...
the $k$th rankit is expected value of the $k$the value.
...
quick-and-dirty method that is very easy to implement:
...
Figure~\ref{} is a quick-and-dirty normal probability plot for
...
Figure~\ref{workspace/brfss_weight_log} shows the distribution
...
Estimators: moments, percentiles.

Apply to pregnancy lengths and weights.
...
The Pareto distibution has an unusual property that we might the
``anti-Wobegon effect.''  Because it is so skewed, the mean is
often much larger than the median, and a large majority of the
population is below the mean.

Find a sample of U.S. incomes.  Compute the median, mean, and
fraction below the mean.  XXX
...
\section{Tail behavior}
  ...
  Values from a Pareto distribution often have
these properties:

\begin{description}

\item[Long tail:] Pareto distributions contain many small values
and a few very large ones.  The part of the distribution that extends
far from the average is called the {\bf tail}.
...
and median, 150 cm.  Figure~\ref{pareto_height} shows what this
distribution looks like.

\begin{figure}
\centerline{\includegraphics[height=2.5in]{workspace/pareto_height.eps}}
\caption{CDF of a Pareto distribution.}
\label{pareto_cdf}
\end{figure}
