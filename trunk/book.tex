...
Create a file named {\tt descriptive.py}.  Write a function called
...
named {\tt Binom} that computes the binomial coefficient.
...
Green, 10\% Orange, 10\% Tan).  Afterwards it was (24\% Blue , 20\%
...
Hint: use the Pdf to compute the relative mean difference
...
% Note: this looks like it's not working!  There's a bug in here
% somewhere.
...
sample contains the values $\{ 1, 2, 5, 7 \}$, the rank of the value 5
...
is assign each of them the same rank, which is the average of
...
transformation from Exercise~\ref{weibull} and then use a linear regression
...
\item Use randomness.  If you divide a large population into two
  groups at random, then for any property, X, you expect the
  difference in the mean of X to be small.  This is a consequence
  ...
  This works even if you don't know what the confounding variables
...
A randomized controlled trial the most reliable way to demonstrate a
causal relationship, and the foundation of science-based medicine (see
\url{wikipedia.org/wiki/Randomized_controlled_trial}).
...
For each live birth in the NSFG dataset, determine the age of the mother
and make a scatter plot of birth weight versus age.  Find the slope
and intercept of the linear least squares fit.
...
% NOTE: bring back the index once there is something in it!
