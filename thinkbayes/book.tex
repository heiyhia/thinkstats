...
The result is $5.5 \cdot 10^{-76}$, which doesn't tell us much except
...
it's not obvious what it means to say that the coin is ``biased.''
...
The likelihood of
$B$ is $34 \cdot 10^{-76}$ and the likelihood ratio is 6.1. So
we would say that the data are evidence in favor of $B$.
...
The likelihood that $x=0.6$ is xxx (which is better than the
likelihood of $F$), but the likelihood of $x=0.4$ is xxx,
which is worse.  So how should we compare this version of $B$
with $F$?

We can think of $B$ as a hypothesis that is made up of two
sub-hypotheses.  We can compute the likelihood for each
sub-hypothesis and the compute the average likelihood.


One natural option is to take average of the likelihoods.
In this case, the weights might be equal

, We can use the data to form the hypothesis

To refresh your memory, here's the {\tt Update} function:
...
After we do an update 


is the normalization constant, which is the
sum of the probabilities

In the prior distribution, the probability of 50\% is 1.  It
gets multiplied by the likelihood, and then the distribution
gets normalized
The {\tt Update} function returns the normalization constant
