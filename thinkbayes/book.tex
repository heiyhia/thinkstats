...
{\tt target} is the integer bucket number corresponding to a
...
likely sources of error, and which we might be able to mitigate.

\begin{itemize}

\item The distribution of growth rates in our simulations are based on
  a continuous model we chose to fit the PMF reported by Zhang et al,
  which is based on 53 patients.  A different sample would yield a
...
\item The distribution of growth rate does not depend on the size of
  the tumor.  This assumption is probably not very good for very
  small and very large tumors, which might be limited by blood supply.
...
\item In the model, the growth rate during each interval is
...
If there is serial correlation, it can't be very strong, because if it
were, we would see a relationship between size at diagnosis and growth
rate.  But there could be a weak correlation, which would affect the
location and spread of the results.

To investigate this possibility, wrote a generator that yields
a series of samples from a given CDF, with a given serial correlation.
...
For example, running simulations with ρ = 0.4 increases the
estimated median age by about a year, and the interquartile range by
about 3 years.  However, if there were a strong serial correlation in
growth rate, there would be also be a correlation between tumor volume
and growth rate, and prior work has shown no such relationship.

There could still be a weak serial correlation, but since there is
currently no evidence for it, I ran these simulations with ρ = 0.

And finally we can give a more precise answer to the question
